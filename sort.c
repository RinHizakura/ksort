...
nevermind:
    ...
    if (nl > 100 && nr > 100) {
        ...
        qs->a = a;
        qs->n = nl;
        /* The struct qsort is used for recursive call, so don't free
...
{
    /* This have to be dynamic allocated so we can always free the pointer
    ...
    queue_work(workqueue, &q->w);
    /* Wait until all the work are completed. We must do this since we depend
     * on stack allocated object, otherwise the work item could access a dead
     * pointer. */
